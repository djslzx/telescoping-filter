 ...
 */
static uint64_t extqf_hash(const ExtQF *filter, elt_t elt) {
  uint64_t buf[2];
 ...
 */
static size_t calc_quot(const ExtQF* filter, uint64_t hash) {
 ...
 */
static rem_t calc_rem(const ExtQF* filter, const uint64_t hash) {
 ...
 */
static uint64_t calc_ext_bits(const ExtQF* filter, uint64_t hash, size_t len) {
 ...
 */
static uint64_t get_ext_code(const ExtQF* filter, size_t block_i) {
  uint64_t code = 0;
 ...
 */
static void set_ext_code(ExtQF* filter, size_t block_i, uint64_t code) {
 ...
 */
static int shortest_diff_ext(ExtQF* filter, uint64_t member_hash, uint64_t non_member_hash, Ext* out) {
...
static int ext_matches_hash(ExtQF* filter, Ext* ext, uint64_t hash) {
  if (ext->len == 0) {
    return 1;
  } else {
 ...
 */
static int select_runend(const ExtQF* filter, size_t block_index, size_t rank) {
  assert(block_index < filter->nblocks && "block_index out of bounds");

  size_t step;
  size_t loc = block_index * 64;
  while (1) {
    ExtQFBlock* b = &filter->blocks[loc / 64];
 ...
 */
static int rank_select(const ExtQF* filter, size_t x) {
  ...
  size_t block_i = x/64;
  size_t slot_i = x%64;
  ExtQFBlock *b = &filter->blocks[block_i];
 ...
 */
static int first_unused(const ExtQF* filter, size_t x) {
  while (1) {
 ...
 */
static void shift_rems_and_runends(ExtQF* filter, int a, int b) {
  if (a > b) return;
  for (int i=b; i>=a; i--) {
 ...
 */
static void shift_remote_elts(ExtQF* filter, int a, int b) {
  if (a > b) return;
  for (int i=b; i>=a; i--) {
 ...
 */
static void shift_block_exts(ExtQF *filter, int block_i, Ext exts[64], const Ext prev_exts[64], int b) {
  uint64_t code;
  for (int i=b; i > 0; i--) {
    exts[i] = exts[i-1];
  }
 ...
 */
static void shift_exts(ExtQF* filter, int a, int b) {
  if (a > b) return;
  uint64_t code;
  if (a/64 == (b+1)/64) {
 ...
 */
static void inc_offsets(ExtQF* filter, size_t a, size_t b) {
  ...
  for (int i = start; i>=0; i--) {
    ExtQFBlock *block = &filter->blocks[i];
 ...
 */
static void inc_offsets_for_new_run(ExtQF* filter, size_t quot, size_t loc) {
...
static void add_block(ExtQF *filter) {
  // Add block to new_blocks
  ExtQFBlock *new_blocks = realloc(filter->blocks, (filter->nblocks + 1) * sizeof(ExtQFBlock));
  if (new_blocks == NULL) {
    printf("add_block failed to realloc new blocks\n");
    exit(1);
  }
  filter->blocks = new_blocks;
  memset(filter->blocks + filter->nblocks, 0, sizeof(ExtQFBlock));
 ...
 */
static void adapt_loc(ExtQF *filter, size_t loc, uint64_t in_hash, uint64_t out_hash) {
  Ext new_ext;
 ...
 */
static void adapt(ExtQF *filter, elt_t query, int loc, size_t quot, rem_t rem, uint64_t hash, Ext exts[64]) {
  ...
  for (int i=loc; i>=(int)quot && (i == loc || !get_runend(filter, i)); i--) {
    ...
    if (remainder(filter, i) == rem && ext_matches_hash(filter, &ext, hash)) {
      ...
      uint64_t in_hash = extqf_hash(filter, filter->remote[i]);
...
void extqf_init(ExtQF *filter, size_t n, int seed) {
  ...
  filter->blocks = calloc(filter->nblocks, sizeof(ExtQFBlock));
...
static void raw_insert(ExtQF* filter, elt_t elt, uint64_t hash) {
  ...
  switch (r) {
    ...
    case RANK_SELECT_OVERFLOW: {
      printf("ExtQF failed to find runend (nslots=%lu, quot=(block=%lu, slot=%lu))\n",
...
static int raw_lookup(ExtQF* filter, elt_t elt, uint64_t hash) {
  size_t quot = calc_quot(filter, hash);
  rem_t rem = calc_rem(filter, hash);
...
int extqf_lookup(ExtQF *filter, elt_t elt) {
  uint64_t hash = extqf_hash(filter, elt);
...
void print_extqf_metadata(ExtQF* filter) {
  ...
  printf("  nslots=%ld, nblocks=%ld, blocksize=%ld, nelts=%ld\n",
         filter->nslots, filter->nslots/64, sizeof(ExtQFBlock), filter->nelts);
  printf("  seed=%d\n", filter->seed);
...
void print_extqf_block(ExtQF* filter, size_t block_index) {
  ...
  ExtQFBlock block = filter->blocks[block_index];
  printf("BLOCK %lu:\n", block_index);
...
void print_extqf(ExtQF* filter) {
  print_extqf_metadata(filter);
  for (int i=0; i<filter->nblocks; i++) {
    print_extqf_block(filter, i);
  }
}
...
#define EXTQF_SEED 32776517

ExtQF *new_extqf(size_t n) {
  ExtQF *filter = malloc(sizeof(ExtQF));
  extqf_init(filter, n, EXTQF_SEED);
  return filter;
}

void test_calc_ext() {
  printf("Testing %s...", __FUNCTION__);
  ExtQF *filter = new_extqf(128);
  // q = 7, r = 8
  uint64_t hash = extqf_hash(filter, 10101);
  ...
  extqf_destroy(filter);
  printf("passed.\n");
}

void test_shortest_diff_ext() {
  printf("Testing %s...", __FUNCTION__);
  ExtQF *filter = new_extqf(64);
  // q=6, r=8
  Ext ext;
  int out;
...
void test_adapt_1() {
  ...
  uint64_t in_hash = extqf_hash(filter, 0);
...
void test_insert_and_query() {
  ...
  for (int i=0; i<s; i++) {
    ...
    extqf_insert(filter, elt);
    assert(set_lookup(str, (int)strlen(str), set, nset));
    assert(extqf_lookup(filter, elt));
  }
  ...
  for (int i=0; i<a; i++) {
    ...
    int in_extqf = extqf_lookup(filter, elt);
    if (in_set) {
      if (!in_extqf) {
        fns++;
        uint64_t hash = extqf_hash(filter, elt);
        ...
        print_extqf_metadata(filter);
        print_extqf_block(filter, quot/64);
        exit(1);
      }
    } else {
      fps += in_extqf;
...
void test_insert_and_query_w_repeats() {
  ...
  for (int i=0; i<n_queries; i++) {
    ...
    sprintf(str, "%lu", elt);
    len = (int)strlen(str);
    int in_filter = extqf_lookup(filter, elt);
